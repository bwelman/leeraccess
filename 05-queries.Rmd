...
```{r q-customername-ordercodes-design-1, fig.cap="Ontwerp query klanten en ordercodes.", out.width="60%"}
...
```{r q-customername-ordercodes-design-2, fig.cap="Ontwerpweergave query Klantnaam+Ordercodes.", out.width="60%"}
knitr::include_graphics("images/queries/q-customername-ordercodes-design.png")
```
...
```{r q-utrecht-dec2009-fields, fig.cap="Velden in het queryraster.", out.width="75%"}
...
```{r q-utrecht-dec2009-criteria, fig.cap="Orderdatum met Like operator.", out.width="75%"}
...
```{r q-customers-cher-design, fig.cap="Ontwerpweergave query Klanten en Kers.", out.width="75%"}
...
```{r q-customers-cher-criteria, fig.cap="Selectiecriterium in ontwerp query, criterium dooscode wordt kers.", out.width="75%"}
...
Met deze knop kun je de rij Totaal in het queryraster zichtbaar en onzichtbaar maken. Access voegt een vak [Totaal]{.uicontrol} toe voor elk veld, net onder het vak [Tabel]{.uicontrol}.

```{r q-pralines-box-design, fig.cap="Query ontwerp met de zichtbare rij totaal.", out.width="75%"}
...
```{r q-boxprice-statistics-design, fig.cap="Ontwerp query doosprijs statistieken.", out.width="75%"}
...
```{r q-show-table, fig.cap="Dialoogvenster Tabel weergeven.", out.width="60%"}
...
3. Voeg achtereenvolgens de tabellen [Orderdetails]{.varname} en [Dozen]{.varname} toe en klik daarna op [Sluiten]{.uicontrol}. De tabellen[Orderdetails]{.varname} en [Dozen]{.varname} zijn nu aan het queryvenster toegevoegd.
...
```{r q-orderrow-amount-design1, fig.cap="Ontwerp met tabellen en velden.", out.width="60%"}
...
```{r q-orderrow-amount-design2, fig.cap="Ontwerp uitgebreid met berekend veld.", out.width="75%"}
...
8. Zorg dat het [Eigenschappenvenster]{.uicontrol} zichtbaar is. Klik ergens in het veld [Regelbedrag]{.varname}.
...
9. Klik in het vak [Notatie]{.uicontrol} en kies dan met de keuzelijst voor `Euro`.

10. Schakel over naar de [Gegevensbladweergave]{.uicontrol}.
...
11. Sluit de query en beantwoord de vraag of de wijzigingen bewaard moeten worden met [Ja]{.uicontrol}. Het venster [Opslaan als]{.wintitle} verschijnt zodat de naam van de query kan worden opgegeven.

12. Typ als naam in [Orderregelbedrag]{.varname} en klik op [OK]{.uicontrol}.
:::
...
2. Kies [tab Maken > Queryontwerp (groep Query's)]{.uicontrol}. Access maakt een nieuw leeg queryvenster en toont het dialoogvenster [Tabel weergeven]{.wintitle} (zie evnetueel figuur \@ref(fig:q-show-table)).

3. Voeg achtereenvolgens de tabellen [Klanten]{.varname} en [Orders]{.varname} toe en klik daarna op [Sluiten]{.uicontrol}. De tabellen zijn nu aan het queryvenster toegevoegd.
...
```{r q-first-order-dates-design1, fig.cap="Tabellen en velden voor query Eerste order per klant.", out.width="60%"}
...
```{r q-first-order-dates-design2, fig.cap="Ontwerp query eerste order per klant.", out.width="60%"}
knitr::include_graphics("images/queries/q-first-order-dates-design2.png")
```

9. Schakel naar de [Gegevensbladweergave]{.uicontrol}.

```{r q-first-order-dates-result, fig.cap="Resultaat query eerste order per klant.", out.width="60%"}
...
3. Voeg de tabellen [Orders]{.varname} en [Orderdetails]{.varname} toe en klik daarna op [Sluiten]{.uicontrol}. De tabellen zijn nu aan het queryvenster toegevoegd.
...
```{r q-information-order-design1, fig.cap="Tabellen en velden voor de query.", out.width="60%"}
...
```{r q-information-order-design2, fig.cap="Tabellen en velden voor de query.", out.width="60%"}
...
```{r q-information-order-result, fig.cap="Resultaat van de query.", out.width="60%"}
...
```{r securitywarning, fig.cap="Berichtenbalk met beveiligingswaarschuwing.", out.width="75%"}
...
3. Voeg de tabel [Bonbons]{.varname} toe en klik daarna op [Sluiten]{.uicontrol}. De tabel is aan het queryvenster toegevoegd.
...
3. Voeg de tabel [Klanten]{.varname} toe en klik daarna op [Sluiten]{.uicontrol}. De tabel is nu aan het queryvenster toegevoegd.
...
```{r q-maketable-dialogbox, fig.cap="Naam specificeren van de nieuwe tabel en de database waarin deze terecht moet komen.", out.width="60%"}
...
2.  Kies [tab Maken > Wizard Query (groep Query's) > Wizard Kruistabelquery > OK]{.uicontrol}. In het scherm dat getoond wordt kun je de tabel of query selecteren welke de velden voor de kruistabel bevat.
...
```{r q-crosstab-rowhead, fig.cap="Selectie velden voor rijkoppen", out.width="60%"}
...
```{r q-crosstab-columnhead, fig.cap="Selectie veld voor kolomkop.", out.width="60%"}
...
```{r q-crosstab-values, fig.cap="Selectie veld en functie voor samenvatting.", out.width="60%"}
