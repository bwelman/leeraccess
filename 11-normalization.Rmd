...
BOEK %>% 
	select(ISBN, Titel, Auteur) %>%
	count(ISBN, Titel, Auteur) %>%
	rename(Aantal = n) %>% 
	arrange(Aantal) %>%
	kbl() %>% 
...
```{r example-dsd1, echo=FALSE, fig.cap="Gegevensstructuurdiagram van de tabellen met de onderlinge relaties."}
grViz(diagram = "digraph dsd1 {
...
```{r example-dsd2, echo=FALSE, fig.cap="Gegevensstructuurdiagram nu met koppelvelden"}
grViz(diagram = "digraph dsd2 {
  graph [rankdir=LR, bgcolor=transparent]
  node [shape = rectangle, fontname = Helvetica]
  KLANT
  FACTUUR
...
```{r animal-clinic-dsd, echo=FALSE, fig.cap="Gegevensstructuurdiagram dierenkliniek."}
grViz(diagram = "digraph case {
  graph [rankdir=LR, bgcolor=transparent]
  node [shape = rectangle, fontname = Helvetica]
  1[label='DIER']
  2[label = 'BEZOEK']
