# (PART) Basis {-}
# Beginnen met Access {#introductie}
...
Access kent de volgende onderdelen ([objecten]{.term}) in een database:
...
```{r example-query, fig.cap="Voorbeeld van een query met ordergegevens.", out.width="60%"}
knitr::include_graphics("images/intro/example-query.png")
```

Een ander soort query is de [actiequery]{.term} die een bepaalde bewerking op de geselecteerde records uitvoert.

### Formulieren {-#intro-form}
...
```{r example-form, fig.cap="Voorbeeld van een formulier voor het invoeren of wijzigen van klantgegevens. Merk op dat voor het invoeren van de Regio is een keuzelijst aanwezig.", out.width="60%"}
knitr::include_graphics("images/intro/example-form.png")
```
...
```{r example-report, fig.cap="Voorbeeld van een deel van een rapport.", out.width="60%"}
knitr::include_graphics("images/intro/example-report.png")
```
...
Met Access beschik je over een ingebouwde programmeertaal, Visual Basic for Applications ([VBA]{.term}). Een module is een verzameling VBA procedures waarmee je complexere databasetaken kunt automatiseren. Voor het maken van een VBA module heb je wel wat programmeerervaring nodig.

## Taak: Access opstarten {#intro-start}
...
## Taak: Access afsluiten {#intro-close}

Om een database af te sluiten kies je [Bestand > Sluiten]{.uicontrol}.

Om Access zelf af te sluiten klik je op de sluitknop van het programmavenster. Dit is een knop met een [X]{.uicontrol} in de rechterbovenhoek van het venster.
...
## Taak: Database openen {#intro-database-open}
...
::: {.practice data-latex=""}
Open via een van de hiervoor genoemde methodes het bestand [snoep365.accdb]{.filepath}.
:::

## Access venster {#intro-access-window}
...
```{r access-window, fig.cap="Programmavenster Access.", out.width="75%"}
...
: + ![Opslaan](images/common/button-save.png) - Opslaan
...
## Nieuwe database venster {#intro-database-new}
...
## Taak: Database opslaan als {#intr-database-saveas}
...
```{r saveas, fig.cap="Dialoogvenster Opslaan als", out.width="60%"}
knitr::include_graphics("images/common/saveas.png")
```
...
### Backup maken {-#intro-database-backup}
...
## Bestandsformaten van Access {#intro-access-fileformats}
