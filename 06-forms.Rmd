...
## Over formulieren maken {#forms-about}
...
### Hoofd- en subformulieren {-#forms-explanation-mainsub}

Een [subformulier]{.term} is een formulier dat in een ander formulier, het [hoofdformulier]{.term}, is opgenomen. Subformulieren worden veel gebruikt wanneer je gegevens weer moet geven uit tabellen en query's met een een-op-veel relatie. De gegevens van de een-kant staan dan in het hoofdformulier en de gegevens uit de veel-kant in het subformulier.
...
```{r f-customers-orders-1, fig.cap="Hoofdformulier Klanten met subformulier Orders.", out.width="50%"}
knitr::include_graphics("images/forms/f-customers-orders.png")
```
:::

### Gekoppelde formulieren {-#forms-explanation-linked}

Gekoppelde formulieren zijn losse formulieren die aan elkaar gerelateerd zijn. Een van de formulieren, ook [hoofdformulier]{.term} genoemd, heeft een [opdrachtknop]{.term} waarmee het andere formulier wordt geopend en de records tussen de twee formulieren worden gesynchroniseerd. Achter de knop zit programmacode ([VBA]{.term}) die er voor zorgt dat het subformulier geopend wordt. Deze VBA code wordt automatisch door de Wizard gemaakt. Je hoeft dus niet te kunnen programmeren om hiermee te werken.
...
```{r f-customerorders-linked-1, fig.cap="Twee formulieren gekoppeld met gesynchroniseerde gegevens.", out.width="75%"}
knitr::include_graphics("images/forms/f-customerorders-linked.png")
```

## Besturingselementen en Indelingen {#forms-controls-layout}
...
```{r layout-controls-tabular, fig.cap="Voorbeeld van een tabelvormige indeling", out.width="50%"}
knitr::include_graphics("images/forms/layout-controls-tabular.png")
```
...
```{r layout-controls-stacked, fig.cap="Voorbeeld van een gestapelde indeling.", out.width="40%"}
knitr::include_graphics("images/forms/layout-controls-stacked.png")
```
...
+ [selecteren]{.term}: Klik op het besturingselement.
+ [meervoudige selectie]{.term}: Klik met ingedrukte Shift toets op de besturingselementen, of sleep met de muis een rechthoekig kader om de besturingselementen.
+ [selectie opheffen]{.term}: Op een lege plek in het formulier klikken.
+ [verplaatsen]{.term}: Versleep het besturingselement via het vierkantje linksboven. Precieze plaats op het formulier kan via het [Eigenschappenvenster]{.uicontrol} ingesteld wordt.
+ [afmetingen]{.term}: Versleep een van de formaatgrepen (in het midden of in de hoekpunten van de selectierechthoek). Precieze afmetingen kunnen via het [Eigenschappenvenster]{.uicontrol} ingesteld worden.
+ [uitlijnen]{.term}: Via de opties bij [tabblad Schikken > groep uitlijning bepalen]{.uicontrol}.
+ [verwijderen]{.term}: Via de [Delete]{.uicontrol} toets.

## Taak: Automatisch formulier {#forms-autoform}
...
```{r f-stock-in-trade-result, fig.cap="Eindresultaat formulier inventaris.", out.width="60%"}
knitr::include_graphics("images/forms/f-stock-in-trade-result.png")
```
...
```{r f-stock-in-trade-design, fig.cap="Ontwerpweergave formulier inventaris.", out.width="60%"}
knitr::include_graphics("images/forms/f-stock-in-trade-design.png")
```
...
```{r textbox-label, fig.cap="Tekstvak met gekoppeld vak bijschrift.", out.width="60%"}
knitr::include_graphics("images/forms/textbox-label.png")
```

Alle bijschriften staan verticaal gerangschikt en alle tekstvakken ook. Dit heet een [Gestapelde indeling]{.term}. Hierdoor kan een enkel besturingselement niet op een willekeurige plaats op het formulier gezet worden. Is dat wel de bedoeling, dan moet je eerst de indeling voor dat besturingselement verwijderen via [tab Schikken > Indeling verwijderen (groep Tabel)]{.uicontrol}.
:::

6. Selecteer de tekst `Dozen` in de [Formulierkoptekst]{.uicontrol} en wijzig deze in [Inventaris]{.userinput}. Wijzig dan de eigenschappen [Tekengewicht]{.uicontrol} in `vet` en [Tekengrootte]{.uicontrol} in `20`.

::: {.info data-latex=""}
Het [Eigenschappenvenster]{.uicontrol} staat meestal aan de rechterkant van het scherm en kan zichtbaar en onzichtbaar gemaakt worden via sneltoets [F4]{.uicontrol}. En om hierin sneller een bepaalde eigenschap te vinden kun je deze alfabetisch sorteren met de knop ![](../common/images/button-sort-ascending.png) rechtsboven in het venster.
:::
...
```{r property-textboxes-width, fig.cap="Instelling eigenschap breedte.", out.width="60%"}
knitr::include_graphics("images/forms/property-textboxes-width.png")
```
...
```{r property-textboxes-locked, fig.cap="Instelling vergrendeling tekstvak via eigenschappenvenster.", out.width="60%"}
knitr::include_graphics("images/forms/property-textboxes-locked.png")
```
...
## Taak: Hoofd- en subformulier {#forms-customers-orders}
...
```{r f-wizard-t-customers-1, fig.cap="Wizard Formulier keuze velden.", out.width="75%"}
knitr::include_graphics("images/forms/f-wizard-t-customers.png")
```
...
```{r f-wizard-customers-orders-dataview, fig.cap="Wizard Formulier keuze weergave.", out.width="75%"}
knitr::include_graphics("images/forms/f-wizard-customers-orders-dataview.png")
```
...
```{r f-customers-orders-2, fig.cap="Formulier klanten en orders.", out.width="60%"}
knitr::include_graphics("images/forms/f-customers-orders.png")
```
...
## Taak: Invoerformulier Klanten {#forms-customers-data-entry}
...
```{r f-customer-entry, fig.cap="Invoerformulier klanten.", out.width="60%"}
knitr::include_graphics("images/forms/f-customer-entry.png")
```
...
## Taak: Invoerformulier Bonbons {#forms-pralines-data-entry}
...
```{r f-praline-entry, fig.cap="Invoerformulier bonbons.", out.width="60%"}
knitr::include_graphics("images/forms/f-praline-entry.png")
```
...
## Taak: Invoerformulier Dozen {#forms-boxes-data-entry}
...
```{r q-choose-praline-design, fig.cap="Ontwerp query voor invoer bonbonkeuze.", out.width="60%"}
knitr::include_graphics("images/forms/q-choose-praline-design.png")
```
...
1. Open het formulier [Dozen subformulier]{.varname} in de [Ontwerpweergave]{.uicontrol}.

2. Verwijder in de sectie [Details]{.uicontrol} het veld [Bonbonnaam]{.varname}.

3. Selecteer [tab Ontwerp > Keuzelijst met invoervak (groep Besturingselementen)] en trek een rechthoekig kader op de plaats van het verwijderde veld [Bonbonnaam]{.varname}. De Wizard [Keuzelijst met invoervak]{.uicontrol} wordt opgestart.

4. Beantwoord de vragen van de Wizard achtereenvolgens als volgt:
...
5. Selecteer het zojuist ingevoerde label dat bij de keuzelijst hoort en druk dan op de [Delete]{.uicontrol} toets om deze te verwijderen.

```{r f-boxes-subform, fig.cap="Keuzelijst op subformulier dozen.", out.width="60%"}
knitr::include_graphics("images/forms/f-boxes-subform.png")
```

6. Selecteer de [Keuzelijst met invoervak]{.uicontrol} en controleer in Eigenschappenvenster (tab Gegevens) de eigenschap [Alleen lijst]{.uicontrol} de waarde `Ja` heeft. Zo niet, dan de waarde wijzigen.
...
7. Sluit het formulier [Dozen subformulier]{.varname} en bewaar de wijzigingen.

8. Test de nieuwe keuzelijst uit.
:::

## Taak: Invoerformulier Orders {#forms-orders-data-entry}

Voor het bekijken van een bestaande order en het invoeren van een nieuwe order moet een orderformulier gemaakt worden, zie figuur \@ref(fig:f-orderdata-entry).

```{r f-orderdata-entry, fig.cap="Invoerformulier orders.", out.width="60%"}
knitr::include_graphics("images/forms/f-orderdata-entry.png")
```
...
```{r q-orders-customers-design, fig.cap="Ontwerp query orders klanten.", out.width="60%"}
knitr::include_graphics("images/forms/q-orders-customers-design.png")
```
...
1. Kies [tab Maken > Queryontwerp (groep Query's)]{.uicontrol} en voeg de tabellen [Orderdetails]{.varname} en [Dozen]{.varname} toe.

2. Voeg van beide tabellen alle velden toe.

3. Maak een kolom met de naam [Totaal]{.varname} die de berekening `Hoeveelheid*Doosprijs` uitvoert.

```{r q-orderdetails-boxes-design, fig.cap="Ontwerp query orderdetails dozen.", out.width="60%"}
knitr::include_graphics("images/forms/q-orderdetails-boxes-design.png")
```

4. Sluit de query en bewaar deze onder de naam [Orderdetails-Dozen]{.varname}.
:::
...
2. Selecteer query [Orders-Klanten]{.varname} en voeg alle velden behalve [Orders.Klantcode]{.varname} toe.

3. Selecteer query [Orderdetails-Dozen]{.varname} en voeg achtereenvolgens de volgende velden toe: [Dozen.Dooscode]{.varname}, [Doosnaam]{.varname}, [Hoeveelheid]{.varname}, [Doosprijs]{.varname} en [Totaal]{.varname}.

4. Klik op [Volgende]{.uicontrol} en laat dan de gegevens weergeven volgens [Orders-Klanten]{.varname}. Hiermee wordt het hoofdformulier gebaseerd op de query [Orders-Klanten]{.varname} en het subformulier op de query [Orderdetails-Dozen]{.varname}.

5. Klik op [Volgende]{.uicontrol} en selecteer dan de opmaak voor het subformulier [In tabelvorm]{.uicontrol}.

6. Kies een stijl en klik dan op [Volgende]{.uicontrol}.

7. Ken de volgende namen toe: [Ordergegevens hoofdformulier]{.varname} en [Ordergegevens subformulier]{.varname} en klik op [Voltooien]{.uicontrol}. Het formulier wordt geopend.

8. Pas desgewenst de lay-out van beide formulieren aan zodat het lijkt op het gewenste formulier voor ordergegevens.
:::

## Taak: Ordertotaal {#forms-orders-total}
...
```{r f-orderdata-entry-totals, fig.cap="Orders met totaalbedrag.", out.width="60%"}
knitr::include_graphics("images/forms/f-orderdata-entry-totals.png")
```
...
```{r f-orderdatasub-properties, fig.cap="Standaardweergave subformulier.", out.width="60%"}
knitr::include_graphics("images/forms/f-orderdatasub-properties.png")
```
...
## Taak: Formulier met grafiek {#forms-sales-boxes}

INFORMATIEBEHOEFTE

Maak een formulier met een kolomgrafiek met de omzet per maand per doos in 2010, zie figuur \@ref(fig:f-sales-boxes-chart)

```{r f-sales-boxes-chart, fig.cap="Formulier met grafiek.", out.width="60%"}
knitr::include_graphics("images/forms/f-sales-boxes-chart.png")
```
...
```{r q-sales-box-month-design, fig.cap="Ontwerp query omzet", out.width="60%"}
knitr::include_graphics("images/forms/q-sales-box-month-design.png")
```
...
1. Kies [tab Maken > Wizard Formulier]{.uicontrol}. Voeg tabel [Dozen]{.varname} toe en van deze tabel alleen het veld [Doosnaam]{.varname}. Klik op [Volgende]{.uicontrol}.

2. Beantwoord de vragen van de Wizard als volgt:
...
3. Open het formulier in de [Ontwerpweergave]{.uicontrol}. Maak wat ruimte voor de grafiek in de sectie [Details]{.uicontrol} door de onderkant naar beneden te slepen.

4. Kies [tab Ontwerp > Grafiek (groep Besturingselementen)]{.uicontrol} ![](images/common/control-chart.png) en teken op het formulier een rechthoekig kader waar de grafiek moet komen. De [Wizard Grafieken]{.uicontrol} wordt opgestart.

5. Beantwoord de eerste drie vragen van de Wizard achtereenvolgens als volgt:
   ...
   + Kies als grafiektype een [Kolomdiagram]{.uicontrol}.
    
   De Wizard toont een voorbeeld van de grafiek. Deze zul je moeten aanpassen.

```{r chartwizard-layout-default, fig.cap="Voorstel indeling grafiek.", out.width="60%"}
knitr::include_graphics("images/forms/chartwizard-layout-default.png")
```

6. Sleep het veld [SomVanMaand]{.varname} uit het vak bij de verticale as naar het vak voor de horizontale as. De weergave langs de horizontale as wijzigt dan in `Maand`.

7. Sleep het veld [Omzet]{.varname} naar het vak voor de verticale as. De weergave langs de verticale as wijzigt in `SomVanOmzet`.

```{r chartwizard-layout-data, fig.cap="Definitieve indeling grafiek.", out.width="60%"}
knitr::include_graphics("images/forms/chartwizard-layout-data.png")
```

8. Klik op [Volgende]{.uicontrol}. Hier selecteer je het veld in het formulier dat gekoppeld moet worden aan het veld in de grafiek. In beide gevallen is dat [Doosnaam]{.varname}.

```{r chartwizard-linking-fields, fig.cap="Koppeling tussen velden formulier en grafiek.", out.width="60%"}
knitr::include_graphics("images/forms/chartwizard-linking-fields.png")
```

9. Klik op [Volgende]{.uicontrol}. Geef de grafiek de naam [Omzet per doos per maand in 2010]{.varname} en kies er voor om de legenda niet weer te geven en klik daarna op [Voltooien]{.uicontrol}.

::: {.warning data-latex=""}
In de Ontwerpweergave wordt een voorbeeldgrafiek getoond en niet een grafiek met de echte gegevens.
:::

10. Sluit het formulier en bewaar de wijzigingen.

11. Open het formulier in de [Formulierweergave]{.uicontrol}, blader door wat dozen en sluit daarna het formulier.
:::

## Taak: Gekoppelde formulieren {#forms-customerorders}
...
De opdracht is om een formulier te maken waarbij de gegevens van een klant getoond worden en dat via klik op een opdrachtknop de ordergegevens van de klant in een nieuw formulier getoond worden. Zie figuur \@ref(fig:f-customerorders-linked-2).

```{r f-customerorders-linked-2, fig.cap="Twee formulieren gekoppeld met gesynchroniseerde gegevens.", out.width="75%"}
knitr::include_graphics("images/forms/f-customerorders-linked.png")
```
...
```{r f-wizard-t-customers-2, fig.cap="Wizard Formulier keuze velden.", out.width="60%"}
knitr::include_graphics("images/forms/f-wizard-t-customers.png")
```
...
```{r f-wizard-customerorders-dataview, fig.cap="Wizard gekoppelde formulieren.", out.width="60%"}
knitr::include_graphics("images/forms/f-wizard-customerorders-dataview.png")
```
...
```{r f-customerorders-titles, fig.cap="Titel hoofdformulier met daaronder de onbereikbare opdrachtknop om het subformulier te openen.", out.width="25%"}
knitr::include_graphics("images/forms/f-customerorders-titles.png")
```
...
## Opgaven {#forms-exercises}

```{r, child='exercises/ex-forms.Rmd'}
```
