# Gegevens toevoegen/wijzigen {#data}
...
## Over gegevens invoeren {#data-about}
...
## Taak: Nieuwe klant toevoegen {#data-new-customer}
...
3. Voer de gegevens in die in de volgende afbeelding te zien zijn.

```{r form-registration, fig.cap="Gegevensinvoer via formulier inschrijving", out.width="75%"}
knitr::include_graphics("images/data/form-registration.png")
```
...
4. Sluit het formulier via de sluitknop [X]{.uicontrol} rechtsboven in het documentvenster. De records die je via het formulier Inschrijving toevoegt worden opgeslagen in de tabel [Klanten]{.varname}. In de volgende stap ga je dit controleren.
...
## Taak: Nieuwe bonbon toevoegen {#data-nnew-praline}
...
5. Sluit formulier [Bonbons]{.varname} via de sluitknop [X]{.uicontrol} rechtsboven in het documentvenster. De records die je via het formulier [Bonbons]{.varname} toevoegt worden opgeslagen in de tabel [Bonbons]{.varname}. In de volgende stap ga je dit controleren.
...
## Taak: Nieuwe doos toevoegen {#data-new-box}

Het formulier [Dozen]{.varname} bevat een subformulier. In het hoofdformulier met de naam [Dozen]{.varname} staat de volgende informatie: [doosnaam]{.varname}, [dooscode]{.varname}, [doosbeschrijving]{.varname}, [gewicht]{.varname}, [doosprijs]{.varname} en de [magazijnvoorraad]{.varname}. Deze gegevens worden opgeslagen in de tabel [Dozen]{.varname}. Bovendien staat op dit formulier een extra veld [Dooskosten]{.varname}. Deze kosten zijn berekend via de som van (`Bonbonkosten * Hoeveelheid`) van alle bonbons die in de doos zitten. In het subformulier met de naam [Dozen subformulier]{.varname} zijn de bewerkbare velden: [Bonboncode]{.varname} en [Hoeveelheid]{.varname}. Deze gegevens, inclusief de dooscode, wordt opgeslagen in de tabel [Doosdetails]{.varname}. In de volgende stappen wordt een nieuwe doos toegevoegd via het formulier [Dozen]{.varname}.
...
8. Voer in bij Doosprijs 25,00 en bij Magazijnvoorraad 120.
...
## Restrictie en validatie van gegevens {#data-restriction-validation}
...
3. Wijzig de Doosprijs van € 27,75 in € 7,75 en druk op TAB toets. Er verschijnt een validatiebericht met de mededeling dat deze prijs niet juist is en hoe we het probleem kunnen verhelpen.
   ...
   Omdat aan het veld doosprijs een validatieregel is gekoppeld, wordt de prijs van 7,75 verworpen.
...
## Opgaven {#data-exercises}

```{r, child='exercises/ex-data.Rmd'}
```
